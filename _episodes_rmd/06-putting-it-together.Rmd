...
in  [episode 4]({{ page.root }}{% link _episodes/04-RMarkdown.md %}).
...
[episode 3]({{ page.root }}{% link _episodes/03-RStudio.md %}). Call the project `old_faithful`.
...
[episode 5]({{ page.root }}{% link _episodes/05-version-control-RStudio.md %}). You should call
the repository `old_faithful`. 

Copy the `old_faithful_updated.Rmd` analysis from [episode 4]({{ page.root }}{% link _episodes/04-RMarkdown.md %}). 
...
### Step 4: Preregister the analysis on OSF.

We're now going to write down our analysis plan and pre-register a simple hypothesis. This will be 
done in three steps: 

1. Create Open Science Framework (OSF) project. 
2. Write an analysis plan (a plan will be provided)
3. Upload documents and pre-register project. 

#### 1. Create OSF project
1. Sign up, if you haven't already for an [osf](osf.io) account. 
2. On the dashboard click `Create new project`. Call it 'Old Faithful' and `Go to project`. 
...
We shall test it by clustering the data using a Gaussian mixture model (more information [here](https://brilliant.org/wiki/gaussian-mixture-model/)) with 1 and 2 components. The true number of components will be given by the model with the highest Integrated Complete Data Likelihood (ICL). 
...
2. `Knit` all the `Rmd` docs to pdfs. 
3. Commit and push changes. 
3. Upload both `analysis_plan.pdf` and `old_faithful_eda.pdf` documents  to the OSF project. 


#### 3. Pre-register plan and project. 
...
1. Download an report template from [here](). 
2. Edit and run the analysis. 
3. Commit and push changes. 
4. Write your own conclusion from the analsis. 
5. Commit and push. 
...
1. `Knit` the `report.Rmd` to a pdf. 
2. Upload the pdf to the OSF project. 


# Conclusions

That concludes the course. In this last episode we have brought together all the elements learned in the previous episodes and applied it to an example analysis of the Old Faithful dataset. We used RStudio to create an project ( [episode 2]({{ page.root }}{% link _episodes/03-RStudio.md %}) ) to contain our Old Faithful analysis. Then we performed some exploratory data analysis and wrote an analysis plan using RMarkdown [episode 3]({{ page.root }}{% link _episodes/04-RMarkdown.md %})). From [episode 1]({{ page.root }}{% link _episodes/01-core-concepts.md %}) we learnt about pre-registration and pre-registered our hypothesis with the OSF. We performed our intended analysis, backing along the way using version control ( [episode 6]({{ page.root }}{% link _episodes/04-version-control-RStudio.md %})). We then published our report on the OSF and because we used version control, our code was already published and available for use by others. 
